...
CHECKPOINT_PATH="${BASE_DIR}/checkpoints/dualencoder-mss-dpr-${CONFIG}-epochs40-trivia"
EMBEDDING_PATH="${BASE_DIR}/embedding-path/psgs_w100-dualencoder-mss-dpr-${CONFIG}-epochs40-trivia.pkl"
...
# Wikipedia evidence path (from DPR code) and Trivia evaluation questions
EVIDENCE_DATA_DIR="${BASE_DIR}/data/dpr/wikipedia_split/psgs_w100.tsv"
QA_FILE_DEV="${BASE_DIR}/data/dpr/retriever/qas/trivia-dev.csv"
QA_FILE_TEST="${BASE_DIR}/data/dpr/retriever/qas/trivia-test.csv"
...
if [ ${TRAIN_WITH_NEG} == "true" ]; then
    # Use this when MSS init is true
    export EXTRA_OPTIONS+=" --train-with-neg --train-hard-neg 7"

    # Use this when just using BERT, no MSS init
