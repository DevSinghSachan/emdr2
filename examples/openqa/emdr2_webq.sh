...
TRAIN_DATA="${DATA_DIR}/webq-train.csv"
VALID_DATA="${DATA_DIR}/webq-dev.csv"
TEST_DATA="${DATA_DIR}/webq-test.csv"
...
CHECKPOINT_PATH="${BASE_DIR}/checkpoints/webq-emdr2-finetuning-${CONFIG}-topk${TOPK}-epochs20-bsize16-async-indexer"
rm -rf ${CHECKPOINT_PATH}

mkdir -p ${BASE_DIR}"/embedding-path/emdr2-finetuning-embedding"

EMBEDDING_PATH="${BASE_DIR}/embedding-path/emdr2-finetuning-embedding/psgs_w100-retriever-webq-emdr2-finetuning-${CONFIG}-topk${TOPK}-epochs20-bsize16-async-indexer.pkl"
rm ${EMBEDDING_PATH}
